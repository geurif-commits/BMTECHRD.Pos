...
public sealed class InventoryMovementsController : ControllerBase
{
    ...
    public InventoryMovementsController(AppDbContext ctx)
    {
        _ctx = ctx;
    }
    ...
    [HttpGet("movements")]
    public async Task<IActionResult> GetMovements([FromQuery] Guid businessId, [FromQuery] Guid? productId, [FromQuery] DateTime? from, [FromQuery] DateTime? to, [FromQuery] int? limit)
    {
        var take = limit ?? 200;
        if (take <= 0) take = 200;
        if (take > 1000) take = 1000;

        var query = _ctx.InventoryMovements.AsQueryable().Where(m => m.BusinessId == businessId);
        ...
        var list = await query
            ...
            .Select(m => new InventoryMovementDto
            {
                ...
                ActorUsername = m.ActorUserId.HasValue ? _ctx.Users.Where(u => u.Id == m.ActorUserId).Select(u => u.Username).FirstOrDefault() : null,
    ...
    {
        var products = await _ctx.Products.Where(p => p.BusinessId == businessId).ToListAsync();
    ...
    public async Task<IActionResult> Adjust([FromBody] InventoryAdjustRequest req)
    {
        ...
        var product = await _ctx.Products.FirstOrDefaultAsync(p => p.Id == req.ProductId && p.BusinessId == req.BusinessId);
