...
public sealed class BarController : ControllerBase
{
    ...
    public BarController(AppDbContext ctx, IHubContext<PosHub> hub)
    {
        _ctx = ctx;
        _hub = hub;
    }
    ...
    public async Task<IActionResult> GetQueue([FromQuery] Guid businessId)
    {
        var items = await _ctx.OrderItems
            .Where(i => i.BusinessId == businessId && i.Area == ProductionArea.BAR && i.Status != OrderItemStatus.CANCELLED && i.Status != OrderItemStatus.DONE)
    ...
    public async Task<IActionResult> UpdateStatus([FromRoute] Guid orderItemId, [FromBody] UpdateOrderItemStatusRequest req)
    {
        ...
        if (item.Area != ProductionArea.BAR) return BadRequest("Item is not for bar");
        ...
        await _hub.Clients.Group(item.BusinessId.ToString()).SendAsync("bar.queue.updated");
        return Ok(item);
    }
}
